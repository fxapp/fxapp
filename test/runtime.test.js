...
describe("makeRuntime", () => {
  ...
  it("should start empty", () =>
    expect(makeRuntime()()).toEqual([undefined, undefined, undefined]));
  ...
  describe("with init and update middleware", () => {
    it("should have initial value and allow for updating", () => {
      ...
      expect(runtime()).toEqual([undefined, undefined, {}]);
      runtime(() => ({ count: 0 }));
      expect(runtime()).toEqual([undefined, undefined, { count: 0 }]);
      runtime(state => ({ count: state.count + 1 }));
      expect(runtime()).toEqual([undefined, undefined, { count: 1 }]);
    });
    it("should perform multiple updates in an array", () => {
      ...
      runtime(() => ({ count: 0 }));
      runtime([inc, inc, inc]);
      expect(runtime()).toEqual([undefined, undefined, { count: 3 }]);
    });
